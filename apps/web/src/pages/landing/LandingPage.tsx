...
export default function LandingPage() {
  ...
  const inputCls = 'w-full border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-colors'
    ...
    <div className="relative w-screen h-screen overflow-hidden bg-dark">
          ...
          <Marker key={v.id} position={[parseFloat(v.latitude), parseFloat(v.longitude)]} icon={makeVendorIcon(getCatIcon(v.category))}>
            <Popup>
              <div className="p-1 min-w-[180px]">
                ...
                <a href={`/vendor/${v.id}`} className="block mt-2 text-center text-xs bg-primary text-white rounded-lg py-1.5 font-medium hover:bg-primary-dark transition-colors">
                  Lihat Profil
                </a>
              </div>
            </Popup>
          ...
          <Marker key={p.id} position={[p.vendors.lat, p.vendors.lng]} icon={makePromoIcon()}>
            <Popup>
              <div className="p-1 min-w-[190px]">
                ...
                <a href={`/vendor/${p.vendors.id}`} className="block text-center text-xs bg-amber-500 text-white rounded-lg py-1.5 font-medium hover:bg-amber-600 transition-colors">
                  Lihat Profil Vendor
                </a>
              </div>
            </Popup>
          ...
          <Marker key={r.id} position={[r.lat, r.lng]} icon={makeRequestIcon()}>
            <Popup>
              <div className="p-1 min-w-[200px]">
                <div className="inline-block bg-teal-100 text-teal-700 text-xs font-bold px-2 py-0.5 rounded-full mb-1.5">
                  🙋 {r.category || 'Umum'}
                </div>
                ...
                <button
                  onClick={() => openBid(r)}
                  className="mt-2 w-full text-center text-xs bg-teal-600 text-white rounded-lg py-1.5 font-medium hover:bg-teal-700 transition-colors"
                >
      ...
      <div className="absolute inset-0 z-10 flex flex-col pointer-events-none">
        ...
        <div className="sm:hidden pointer-events-auto px-3 pt-4 pb-2 flex flex-col gap-2">
          <div className="flex items-center gap-2">
            ...
            <Link to="/mitra/login" className="bg-dark-card/90 backdrop-blur-sm text-white border border-dark-border px-3 py-2 rounded-xl text-xs font-medium hover:border-primary transition-colors shadow-lg whitespace-nowrap">
              Masuk
            </Link>
            <Link to="/mitra/register" className="bg-primary text-white px-3 py-2 rounded-xl text-xs font-semibold hover:bg-primary-dark transition-colors shadow-lg whitespace-nowrap">
              Daftar
            </Link>
          </div>
        ...
        <div className="hidden sm:flex pointer-events-auto items-center gap-3 px-4 pt-4 pb-2">
          ...
          <div className="flex gap-2 shrink-0">
            <Link to="/mitra/login" className="bg-dark-card/90 backdrop-blur-sm text-white border border-dark-border px-4 py-2.5 rounded-2xl text-sm font-medium hover:border-primary transition-colors shadow-lg">Masuk</Link>
            <Link to="/mitra/register" className="bg-primary text-white px-4 py-2.5 rounded-2xl text-sm font-semibold hover:bg-primary-dark transition-colors shadow-lg">Daftar Vendor</Link>
          </div>
        </div>
        ...
        <div className="pointer-events-auto px-3 sm:px-4">
          <div className="inline-flex items-center gap-3 bg-dark-card/80 backdrop-blur-sm px-3 py-1.5 rounded-full border border-dark-border text-xs text-white/60">
      ...
      {selectedRequest && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50" onClick={() => setSelectedRequest(null)}>
          <div className="bg-white w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            {bidDone ? (
              <div className="text-center py-4">
                ...
                <button onClick={() => setSelectedRequest(null)} className="w-full bg-primary text-white py-3 rounded-xl font-semibold text-sm">
                  Tutup
                </button>
              </div>
            ) : (
              <>
                ...
                <div className="bg-teal-50 rounded-xl p-3 mb-4">
                  <span className="inline-block bg-teal-100 text-teal-700 text-xs font-bold px-2 py-0.5 rounded-full mb-1">{selectedRequest.category || 'Umum'}</span>
                ...
                <div className="flex gap-3">
                  <button onClick={() => setSelectedRequest(null)} className="flex-1 border border-gray-200 text-gray-500 py-3 rounded-xl text-sm font-medium hover:bg-gray-50">
                    Batal
                  </button>
                  <button onClick={submitBid} disabled={bidSubmitting || !bidPrice}
                    className="flex-2 flex-1 bg-teal-600 hover:bg-teal-700 disabled:opacity-50 text-white py-3 rounded-xl text-sm font-bold transition-colors">
